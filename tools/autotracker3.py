...
############## COMMENTS, YO ##############
#I will be commenting any lines where changing them makes a difference in the song. look fot "YAY"
#I will also comment lines that might seem cool to change, but don't seem to do anything useful. look for "FFS"

import sys, struct, random, os, json
import math
#import randomsongname as GEN

# Seed before class-level random calls at definition time
if len(sys.argv) > 1:
...
class ITFile:
    def __init__(self):
        ...
        self.gvol = 128 #FFS
        self.mvol = 48 #FFS
        self.tempo = random.randint(90,212) #YAY Raising this makes the song faster. max is 255.
        #Raised max from 180 to 220.
        self.speed = random.randint(2,5) #4 #YAY: Raising this actually slows the songs down.
        # 2 is the lowest you should go, basically speedcore. 10 is probably the highest you should go.
        #
        self.pitchwheel = 1 #FFS
        self.pansep = 128
        self.message = (
    ...
    def doheader(self, fp):
        ...
        fp.write(b"AtBu")
        fp.write(bytes(self.chnpan))
        fp.write(bytes(self.chnvol))

        fp.write(bytes(ordlist))
    ...
    def write_padded(self, length, s, addnull=True):
        ...
        assert len(b) == length, "STUPID! write_padded gave wrong length!"
...
class Sample_KS(Sample):
    ...
    boostlv = random.randint(1,10)
    if boostlv < 9:
        boost = random.randint(1,4)
    else: #We are the 10%
        boost = 100000000000#random.randint(1,4)#YAY: This just makes the sounds a bit more abrasive. Or sometimes a lot.
...
class Sample_Kicker(Sample):
    ...
    def generate(self):
        ...
        q_noise = 0.0
#THIS CHANGES THE KICK PITCH THATS SO COOL
        kickmul = math.pi*2.0*random.randint(10,2000)/random.randint(2,6)/SMP_FREQ
        offs_sine = 0.0
...
RN_NOUNS = [
    ...
    ("tofu","tofus"),("vine","vines"),("branch","branches"),("dong", "dongs"),
    ("david","davids"),("fart","farts"),("art","arts"),("santa", "santas"),
    ("satan","satans"),("weed","weeds"),("rainbow","rainbows")
...
RN_ADJECTIVES = [
    ...
    "woeful","tearful","rich","poor","awoken","sacred","high","floppy","derpy",
    "rigid","chill-AF","constipated","gross","massive","ungrateful","shiny",
    "colorful","gay","scared","rainbow",
]
...
def randInt1b0a():
    ...
    #               #
    #################
def createSong(dead=False):
    if dead:
        fname = "savedmusic\\walking-and-walking.it"
        return fname
    else:
        MIDDLE_C = random.randint(27, 2200)#2196.0# * (2.0 ** (3.0 / 12.0))
        #YAY: Changing this changes the main notes of the song... seems to be pretty decent already.
        #setting it to something obscene like 5000 makes the notes basically inaudible.
        #Raised max from 1800 to 2200
        ...
        while True:
            #sometimes the random numbers cause an error... If this happens, just try again until they don't
            try:
                SMP_GUITAR = itf.smp_add(Sample_KS(name = "KS Guitar", freq = MIDDLE_C/2, decay = float("0.00" + str(randInt1b0a())), nfrqmul = 1.0, filt0 = randInt0b2a(), filtn = 0.6, filtf = float("0.000" + str(randInt1b0a())), length_sec = randInt0b1a()))
                SMP_BASS = itf.smp_add(Sample_KS(name = "KS Bass", freq = MIDDLE_C/4, decay = float("0.00" + str(randInt1b0a())), nfrqmul = 0.5, filt0 = randInt0b2a(), filtn = 0.2, filtf = float("0.00" + str(randInt1b0a())), length_sec = randInt0b1a()))
                ...
                SMP_HHC = itf.smp_add(Sample_NoiseHit(name = "NH Hihat Closed", gvol = random.randint(1,255), decay = 0.03, filtl = 0.99, filth = 0.20))
                SMP_HHO = itf.smp_add(Sample_NoiseHit(name = "NH Hihat Open", gvol = random.randint(1,255), decay = 0.5, filtl = 0.99, filth = 0.20))
                ...
                print ("invalid song configuration, trying again")
                continue
            else:
                break
        ...
        #name = random.choice([randoname(), GEN.generate()])
        itf.name = name
        songs_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "songs")
        os.makedirs(songs_dir, exist_ok=True)
        fname = os.path.join(songs_dir, "%s.it" % name.replace(" ","-").replace("'",""))
        if len(sys.argv) > 2:
            fname = sys.argv[2]
        itf.save(fname)

        # Write timing manifest for the rhythm game
        manifest = {
            ...
            'patsize': 128,
            'numPatterns': 6,
            'songFile': '/games/dancing-and-dancing/songs/' + os.path.basename(fname),
        }
        json_path = os.path.splitext(fname)[0] + '.json'
        with open(json_path, 'w') as jf:
            json.dump(manifest, jf)

        #print "Done"
        print ("Song Name: \"%s\"" % fname)
        return fname
